...
class GroupChatView extends GetView<MessageController> {
    ...
    ever(controller.shouldScrollToBottom, (shouldScroll) {
      if (shouldScroll && scrollController.hasClients) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          developer.log(
            'Scrolled to bottom, offset: ${scrollController.offset}',
            name: 'GroupChatView',
          );
        });
      }
            ...
            Expanded(
              child: Obx(() {
                ...
                return ListView.builder(
                  ...
                  itemCount:
                      roomMessages.length +
                      (controller.isLoadingMoreMessages.value ? 1 : 0),
                  itemBuilder: (context, index) {
                    if (index == roomMessages.length &&
                        controller.isLoadingMoreMessages.value) {
                    ...
                    final sender = participants.firstWhere(
                      (p) => p['id'] == senderId,
                      orElse: () => {'name': 'Unknown', 'image': null},
                    );
                    final senderName = sender['name'] as String? ?? 'Unknown';
                    final senderAvatar = _resolveMediaUrl(
                      sender['image'] as String?,
                    );

                    if (type == 'post') {
                      return _buildSharedPostBubble(
                        msg['post_id'] ?? 0,
                        senderName,
                        senderAvatar,
                      );
  ...
  Widget _buildGroupAvatar() {
    final avatars = participants.take(2).map((p) {
      return _resolveMediaUrl(p['image'] as String?);
    }).toList();
        ...
        children: [
          Positioned(
            ...
            child: CircleAvatar(
              ...
              backgroundImage: avatars.isNotEmpty && avatars[0].isNotEmpty
                  ? NetworkImage(avatars[0])
                  : null,
            ),
          ),
          Positioned(
            ...
            child: CircleAvatar(
              radius: 18,
              backgroundColor: Colors.grey[300],
              backgroundImage: avatars.length > 1 && avatars[1].isNotEmpty
                  ? NetworkImage(avatars[1])
                  : null,
            ),
          ),
        ],
